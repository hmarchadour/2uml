...
public class JModelVisitorImpl implements JModelVisitor {
	...
	public JModelVisitorImpl(JDTHandler handler) {
	...
	public void visit(IJavaElement javaElement) {
		if (javaElement != null && !visited.contains(javaElement)) {
			if (stack.size() < recursiveLimit) {
				...
				CoreActivator.log(IStatus.ERROR, error.toString());
			}
		}
	}
	...
	protected void visitMember(IMember javaElement) throws JavaModelException {
		if (javaElement instanceof IField) {
			...
			ITypeWrapper type = CoreFactory.toWrappedType((IType)javaElement);
